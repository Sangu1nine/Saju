...
export const startStreamingAnalysis = async (sajuData, onProgress) => {
  ...
  try {
    // 데이터 타입 변환 및 필수 필드 보정
    const requestData = {
      birth_year: parseInt(sajuData.birth_year),
      birth_month: parseInt(sajuData.birth_month),
      birth_day: parseInt(sajuData.birth_day),
      birth_hour: parseInt(sajuData.birth_hour),
      birth_minute: parseInt(sajuData.birth_minute || 0),
      is_male: sajuData.is_male !== undefined ? sajuData.is_male : true,
      city: sajuData.city || "서울",
      is_lunar: Boolean(sajuData.is_lunar),
      is_leap_month: Boolean(sajuData.is_leap_month)
    }
    
    console.log('🚀 8단계 실시간 스트리밍 분석 시작:', requestData)
